...
Page({
  ...
  getMovieListDatas: function(url, settedKey, categary) {
    wx.request({
      url: url,
      header: {
      ...
      success: (res) => {
        if (categary) {
          this.processMovieData(res.data, settedKey, categary);
        } else {
          this.processSearchMovieData(res.data, settedKey);
        }
      },
      fail: function (res) {
        console.log(res);
  ...
  processMovieData: function (data, settedKey, categary) {
    var movies = [];
    var group01 = [];
    var group02 = [];
